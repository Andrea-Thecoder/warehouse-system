...
### Package
Tutto sotto `it.warehouse.optimization.*`. I sotto-package corrispondono ai layer.
...
- **Formato obbligatorio**: ogni messaggio inizia con il nome del metodo corrente, separato dal testo da `: `, sempre in inglese, senza caratteri speciali

```java
log.info("register: new registration request for username {}", dto.getUsername());
log.debug("buildDisabledUser: building user representation for {}", dto.getUsername());
log.error("approve: failed to enable user {} on Keycloak", keycloakUserId);
```
